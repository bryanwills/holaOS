...
test("app shell uses the integrated title bar path for macOS and Windows", async () => {
  ...
  assert.match(
    source,
    /const titleBarContainerClassName =\s*desktopPlatform === "win32"\s*\?\s*"relative min-w-0 -mx-2 -mt-1 sm:-mx-3 sm:-mt-1.5"/,
  );
  assert.match(
    source,
...
test("app shell wires filesPaneWidth into the explorer panel and uses a drag handle mirrored from the agent pane resizer", async () => {
  ...
  assert.match(source, /const startExplorerPanelResize = useCallback\(/);
  assert.match(source, /beginExplorerDrag\(event, filesPaneWidth\);/);
  assert.match(source, /setExplorerRevealDragWidth\(/);
  assert.match(source, /setFilesPaneWidth\(clampExplorerPanelWidth\(current\)\);/);
  assert.match(
    source,
