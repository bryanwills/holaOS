...
test("desktop file preview supports tabular spreadsheet kinds", async () => {
  ...
  assert.match(source, /const nodeRequire = createRequire\(__filename\);[\s\S]*const XlsxPopulate = nodeRequire\("xlsx-populate"\) as XlsxPopulateStatic;/);
  ...
  assert.match(source, /const hyperlink = worksheet\.cell\(rowIndex \+ 1, columnIndex \+ 1\)\.hyperlink\(\);/);
  assert.match(source, /return normalizePreviewTableLinkTarget\(hyperlink \?\? value\);/);
  assert.match(source, /links,\s*totalRows: allRows\.length,/);
  assert.match(source, /cell\.value\(value\);[\s\S]*if \(hyperlink\) \{[\s\S]*cell\.hyperlink\(hyperlink\);[\s\S]*\} else \{[\s\S]*worksheet\.hyperlink\(cell\.address\(\), undefined\);[\s\S]*\}/);
  assert.match(source, /async function writeTableFile\(/);
  assert.match(source, /function parseCsvRows\(text: string\): string\[\]\[\]/);
  assert.match(source, /function stringifyCsvRows\(rows: string\[\]\[\]\): string/);
