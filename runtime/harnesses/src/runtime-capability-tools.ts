...
function runtimeToolPromptGuidelines(toolId: RuntimeAgentToolId): string[] {
  ...
  if (toolId === "holaboss_workspace_integrations_propose_connect") {
    return [
      ...
      "INTEGRATION-VS-APP-BINDING — Integrations are user-level OAuth accounts; a single connected Gmail account can power any number of apps. Apps consume those accounts via a per-app binding. Two different chat cards exist for the two situations: a 'Connect <provider>' card (this tool) opens the OAuth flow for a brand-new account; a 'Pick a <provider> account for <app>' card auto-renders from `pending_integrations` and lets the user bind an existing connection to the app. Only call this tool when the user genuinely has zero accounts for the provider.",
      "Call this tool ALSO whenever you've just built or registered an app that needs a provider and the user hasn't connected it. `pending_integrations` in the response of `workspace_apps_register` / `workspace_apps_ensure_running` is the explicit signal — but only for entries where `available_accounts === 0`. If `available_accounts >= 1`, the chat UI already renders the per-app binding picker; calling propose_connect on top of that creates a duplicate Connect card and confuses the user. Do NOT ship a 'safe mode' / 'manual mode' / 'no real recipient' fallback to avoid asking.",
