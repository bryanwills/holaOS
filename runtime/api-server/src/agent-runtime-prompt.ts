...
export function buildMainSessionPromptSections(
  ...
  if (normalizedSessionKind === "onboarding") {
    ...
    conversationLines.splice(4, 0,
      ...
      "Connect-an-integration intent is its own primitive — separate from building an app. When the user asks to connect, link, sign in, authorize, or 'use my <product>' for any third-party service, FIRST call `workspace_integrations_list_catalog` to resolve the canonical `toolkit_slug` (the product's marketing name may differ — Twitter/X, Google Drive/googledrive, etc.), then call `holaboss_workspace_integrations_propose_connect` with that slug. The Connect card it emits is the entire UX for this case — once the user authorizes, the `<toolkit>_<verb>` tools become available automatically on the next turn via the managed composio-mcp host.",
