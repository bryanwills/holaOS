    ...
    case "subagent":
      ...
      if (hasWorkspaceIntegrationCatalogTool(request)) {
        lines.push(
          ...
          "If the delegated task is to CONNECT a third-party service (not build an app), do NOT scaffold an app or edit `workspace.yaml` `mcp_registry`. Return up to the parent with the toolkit slug so the parent main session can emit the Connect card via `holaboss_workspace_integrations_propose_connect`; this subagent does not surface Connect cards itself."
        );
      }
      break;
    case "main_session":
...
export function buildMainSessionPromptSections(
  ...
  if (normalizedSessionKind === "onboarding") {
    ...
    conversationLines.splice(4, 0,
      ...
      "Connect-an-integration intent is its own primitive — separate from building an app. When the user asks to connect, link, sign in, authorize, or 'use my <product>' for any third-party service, FIRST call `workspace_integrations_list_catalog` to resolve the canonical `toolkit_slug` (the product's marketing name may differ — Twitter/X, Google Drive/googledrive, HubSpot/hubspot, etc.), then call `holaboss_workspace_integrations_propose_connect` with that slug. The catalog includes both OSS-adapter providers (`supports_oss: true`) and managed-only providers (`supports_oss: false`); both are valid `propose_connect` targets — `supports_oss: false` only restricts in-app `integrations:` manifest usage, NOT the connect flow. The Connect card it emits is the entire UX for this case — once the user authorizes, the `<toolkit>_<verb>` tools become available automatically on the next turn via the managed composio-mcp host. Never tell the user 'this provider isn't in the catalog' or fall back to a browser walkthrough until you have confirmed the slug is absent from `workspace_integrations_list_catalog.provider_ids`.",
      "Connecting a toolkit does NOT require building, scaffolding, or registering any app, and does NOT require editing `workspace.yaml`. Treat the toolkit's catalog slug as the only thing the agent must know; OAuth and tool wiring are runtime concerns. Build an app only when the user explicitly asks for one (\"build / make / scaffold an app that uses X\"); connect alone is sufficient otherwise.",
      "Antipattern — do NOT improvise the connect flow by adding `mcp_registry.servers.<toolkit>` entries to `workspace.yaml` via shell/file-edit tools, by scaffolding an app whose only purpose is to host a remote MCP URL, or by hand-rolling a Connect card in chat. Public/managed MCP URLs (e.g. `https://mcp.<product>.com/mcp`) require auth credentials the agent does not have, will fail with `Invalid content type` / SSE transport errors at the MCP host, and will leave the workspace in a broken state.",
