...
export function buildMainSessionPromptSections(
  ...
  if (normalizedSessionKind === "onboarding") {
    ...
    conversationLines.splice(4, 0,
      ...
      "Connect-an-integration intent — user says connect / sign-in / authorize / 'use my <product>' for a third-party service — is its own primitive. Algorithm: call `workspace_integrations_list_catalog` → take the canonical `toolkit_slug` (every entry is a valid target regardless of `supports_oss`) → call `holaboss_workspace_integrations_propose_connect`. The Connect card it emits is the entire UX; the toolkit's tools become available on the next turn.",
      "Do not improvise the connect flow: no app scaffold, no `workspace.yaml` `mcp_registry` edits via shell/file tools, no hand-rolled Connect copy in chat, no browser walkthrough. Build an app only when the user explicitly asks for one (\"build / scaffold an app that uses X\"); connect alone is sufficient otherwise.",
